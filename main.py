...
import os
# database import
from data import db_session, styles_api
...
@app.route('/get_style_api', methods=["GET", "POST"])
def proccess_style():
    filtered_colors = tuple(filter(lambda x: current_settings["font-colors"][x] == 'selected',
                             current_settings["font-colors"].keys()))
    req_color = filtered_colors[0]
    filtered_families = tuple(filter(lambda x: current_settings["fonts"][x][0] == 'selected',
                                     current_settings["fonts"].keys()))
    req_font_family = filtered_families[0]
    return redirect(url_for('style_api.get_style', font_size=int(current_settings['font-size']), font_color=req_color,
                            font_family=req_font_family))


def launch_app():
    create_folder()
    app.register_blueprint(styles_api.blueprint)
    app.run(port=8080)
